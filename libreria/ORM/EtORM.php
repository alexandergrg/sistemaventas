...
class EtORM extends \Conexion
{
  ...
  function __construct(){
    //Incocar a al función para ejecutarse el metodo
    self::getConexion();
  }
  ...
  public static function getDesconectar(){
      ...
      public function eliminar($valor = null , $columna = null){
           $query = "DELETE FROM ". static ::$table ." WHERE ". (is_null($columna) ? "id" : $columna) . " = :p";
            //echo $query;
            //$query = 'DELETE FROM ventas WHERE id =' . 5 ;
            self::getConexion();
            //preparar conexión
            $res = self::$cnx->prepare($query);
    ...
    public function guardar(){
      ...
      public static function find($id , $parametroKey = null){
          //echo get_called_class();
          //echo $parametroKey;
          $resultado = self::where($parametroKey == (null) ? 'id' : $parametroKey ,$id);
          if(count($resultado)){
              return $resultado[0];
          }else{
              return [];
